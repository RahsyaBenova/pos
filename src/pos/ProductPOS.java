...
public class ProductPOS {
    ...
    public ProductPOS() {
        try {
            conn = ConnectionDatabase.getInstance().getConnection();
    ...
    // FIXED: Removed shared instance variable assignment and return type bug
    ...
    // FIXED: Changed query target to customers table and properly returned custCode
    public String getCustCode(String custName) {
        String custCode = null;
        try {
            String query = "SELECT customercode FROM customers WHERE fullname='" +custName+ "'";
            resultSet = statement.executeQuery(query);
            while (resultSet.next()) {
    ...
    // FIXED: Converted flag to a local variable to prevent persistent cross-lifecycle bugs
...
"                    ON salesinfo.soldby=users.username;";
    ...
    // Search method for purchase logs (FIXED: Added space before WHERE)
